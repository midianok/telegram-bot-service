...
if (process.env.WWEBHOOK_URL){
    bot.launch({ webhook: { domain: process.env.WEBHOOK_URL, port: Number(process.env.WEBHOOK_PORT) || 80 } })
        .then(() => logger.info(`App started webhook "${process.env.WEBHOOK_URL}" listening on port ${process.env.WEBHOOK_PORT}`));
}
else {
    bot.launch().then(() => logger.info(`App started with polling`));
}
